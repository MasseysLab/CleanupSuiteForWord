...
- Current long-lived local working branch: `codex-0.7.0-start`
- Important: the branch name is legacy and does not mean the project is still at `0.7.0`.
- When discussing current state, trust the checked-in source/version metadata before trusting older branch naming or older handoff entries.
...
## 2026-06-17 22:43 - Tool-form rules implementation pass

Goal:
- Implement Chris's `docs\Tool-Form Rules.md` instructions across the tool forms while keeping that rules file exactly as-is.

Backup:
- Created before editing: `backups\CleanupSuite_backup_20260617_223549_before_tool_form_rules.zip`.

Changes made:
- Removed tool-form Help button behavior from individual tool forms; main-menu help buttons remain available.
- Updated shared guided tool layout so every tool form uses the same order: body title, centered bold heading, choices, conditional custom divider/custom choices/select buttons, informational box, Preview, Scope when available, then Apply/Reset.
- Kept the top-left body title visible and continued suppressing the form title-bar title.
- Added shared informational-box rendering from live form choices, with selected choices bolded.
- Added control click refresh handlers so checkbox/radio/select-all changes redraw the shared layout and informational box.
- Changed tool-form action layout to two equally sized bottom buttons (`Apply` and `Reset`) and hides `cmdHelp`.
- Updated installer generation rules to match the no-tool-help-button/two-action-button layout.

Sync status:
- Ran `python assemble.py`; generated root bundle and practice bundle were rebuilt.
- Root/practice `VBA_Cleanup_tool.txt` SHA-256 hashes match: `C038B437170525D08BCD1AA208AF17BE9D68CF9CCBBC0A19A7697B158D7E945F`.
- `docs\Tool-Form Rules.md` remains untracked/user-owned and was not edited.

Verification:
- `python assemble.py` -> OK (`24 builders | balance, if-traps, wiring, strings, round-trip all clean`).
- `python -m unittest discover -s tests` -> 42 tests OK.
- Search confirmed no individual tool form calls `ShowCleanupToolHelp`; only the main launcher retains shared help calls.

## 2026-06-17 23:18 - Tool-form rules correction pass

Goal:
- Correct two rule misses from the previous tool-form pass: tool-form Help buttons were still possible through stale generated/layout paths, and informational boxes were being filtered by control visibility so they could appear to show only selected/current choices.

Changes made:
- Removed the obsolete capitalization-only layout routine that still positioned a tool-form `cmdHelp` button.
- Updated the shared guided-layout control skip list so `cmdHelp` is not treated as a layout participant for tool forms.
- Updated installer generation to remove `cmdHelp` controls from tool forms during reinstall while preserving the main-menu help buttons.
- Updated guided informational-box filtering to stop using `ctl.Visible` as the inclusion test; it now uses the tool-form rule visibility helper instead, so all relevant main/custom choices can be listed while hidden dependent controls stay out.
- Updated regression tests so tool forms are protected from Help buttons without blocking the main-menu help system.

Sync status:
- Ran `python assemble.py`; generated root bundle and practice bundle were rebuilt and synced.

Verification:
- `python assemble.py` -> OK (`24 builders | balance, if-traps, wiring, strings, round-trip all clean`).
- `python -m unittest discover -s tests` -> 42 tests OK.
- Search confirmed source/root/practice bundles no longer contain the stale tool-form `cmdHelp` layout, stale capitalization layout, or `If Not ctl.Visible Then Exit Function`.

## 2026-06-17 23:46 - Embedded DOCM code resync

Goal:
- Finish the correction pass by syncing the already-verified source/generated changes into the embedded Word hosts so Word itself reflects the no-tool-help / full-info-box behavior.

Backup:
- Created before embedded document edits: `backups\pre_docm_sync_20260617_233944`.

Changes made:
- Added reusable helper script: `scripts\sync_docm_code_only.ps1`.
- Synced code modules on temp copies, then wrote the updated copies back to:
  - `CleanupSuite_Workspace.docm`
  - `Practice - Try CleanupSuite Here\CleanupSuite_Workspace.docm`
- Preserved existing UserForm designers instead of running the full installer flow.

Verification:
- Embedded read-back from both `.docm` files confirmed:
  - `modCleanupHelpers` contains `ShouldHideGuidedControl(...)`
  - old `If Not ctl.Visible Then Exit Function` is absent
  - new `If ShouldHideGuidedControl(toolForm, ctl.Name) Then Exit Function` is present
  - old `LayoutCapitalizationCleanupForm` path is absent
  - `HideGuidedControl toolForm.Controls("cmdHelp")` is present
  - `frmCapitalizationCleanup` and `frmObjectRemover` do not contain `cmdHelp_Click`

Notes:
- This was a code-only embedded sync, matching the earlier safe pattern documented in the handoff: temp-copy Word COM editing plus direct VBProject code replacement.

## 2026-06-18 - Guided tool-form layout correction pass

Goal:
- Apply the latest tool-form layout decisions without manually chasing each form: Select/Deselect should only appear with active custom multi-select choices, Header / Footer should follow its special row order, and the shared guided layout should stay compact.

Changes made:
- Updated `LayoutCleanupToolForm` shared spacing so the intro, divider, info box, scope, and bottom button area are tighter.
- Updated `ShouldHideGuidedControl` so `cmdSelectAll` and `cmdDeselectAll` are hidden unless Custom mode is active.
- Updated `frmCapitalizationCleanup` so its own visibility refresh also hides Select/Deselect unless Custom is selected.
- Updated Header / Footer generated control order so `Unlink sections` comes before `Set alignment`, and the alignment choices order as Left, Right, then centered Center.
- Updated `docs\Tool-Form Rules.md` to preserve the current tool-form rules, including the model/reasoning reminder, compact spacing rule, practice sync rule, no tool-form Help buttons, body-title-only rule, and Header / Footer special layout rule.

Sync status:
- Ran `python assemble.py`; generated root bundle and practice bundle were rebuilt.
- Root/practice `VBA_Cleanup_tool.txt` SHA-256 hashes match: `A63850CC03FF94D49F7194DA0A8566BF2833E05ABF463AD39D1229D4BF4FF462`.
- Ran `scripts\sync_docm_code_only.ps1`; synced both `CleanupSuite_Workspace.docm` and `Practice - Try CleanupSuite Here\CleanupSuite_Workspace.docm`.

Verification:
- Focused regression tests for Select/Deselect visibility and Header / Footer ordering passed.
- `python assemble.py` -> OK (`24 builders | balance, if-traps, wiring, strings, round-trip all clean`).
- `python -m unittest discover -s tests` -> 43 tests OK.

## Session: 2026-06-18 16:32
Agent: Codex

Goal:
- Finish the shared guided tool-form polish pass by removing stale Custom divider chrome after Custom is turned off and tightening whitespace across the shared layout.

Backup created:
- `backups\CleanupSuite_backup_2026-06-18_16-15-20.zip`

Git/repo state:
- Remote: `origin https://github.com/MasseysLab/CleanupSuiteForWord.git`
- Branch: `codex-0.7.0-start`
- Status before: dirty working tree with the existing 0.8-era tool-form/layout/docm changes already in progress.
- Status after: same broader dirty working tree, plus the shared layout reset/compact-spacing correction and synced generated/docm artifacts.
- GitHub/local/generated/.docm sync checked: yes for local source, root/practice generated bundles, and both local `.docm` workspace files.
...
Summary of changes:
- Added a shared reset pass for generated guided-layout chrome so `lblGuidedDivider`, the info-box border, and generated info lines are explicitly hidden before every relayout.
- Fixed the stale Custom divider behavior for Custom-capable tools by clearing generated chrome before the shared layout rebuilds the visible controls.
- Tightened the shared guided layout vertically by reducing title/introduction offsets, inter-section gaps, info-box padding, option heights, and trailing form height padding.
- Rebuilt the generated root/practice installer bundles and synced the updated VBA code into both local `.docm` hosts with the code-only Word COM sync script.

Important observations:
- The lingering divider bug came from generated labels being skipped during relayout; once created, they could remain visible unless explicitly hidden first.
- The whitespace complaint is primarily a shared-layout issue, so this pass intentionally corrected the common helper rather than chasing spacing separately in each tool form.

Tests/checks run:
- `python -m unittest tests.test_guided_tool_dynamic_layout -q` -> OK after first writing failing regression expectations for generated chrome reset and compact spacing.
- `python -m unittest discover -s tests -q` -> 45 tests OK.
- `python assemble.py` -> OK (`24 builders | balance, if-traps, wiring, strings, round-trip all clean`).
- `python vbaeval.py validate` -> OK.
- Root/practice `VBA_Cleanup_tool.txt` SHA-256 hashes match: `A05AF974...`.
- `powershell -ExecutionPolicy Bypass -File scripts\sync_docm_code_only.ps1` -> OK for both workspace `.docm` files.

Known remaining issues:
- No fresh manual Word click-through was performed in this pass, so the final visual judgment on “tight enough” still depends on opening the actual forms in Word.
- `docs\Tool-Form Rules.md` and `scripts\` remain untracked/user-owned in the working tree.

Next recommended step:
- Open the simple and Custom tools in Word, confirm the tighter spacing feels right and the Custom divider disappears when Custom is turned back off, then commit the current branch once the visual pass is approved.
