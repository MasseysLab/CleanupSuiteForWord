...
> Documentation basis: CleanupSuite 0.8.5 Alpha as currently prepared in this repository. This guide reflects the current source-first workflow, shared guided-form layout, and current launcher behavior.
...
Do not use `Me.Hide: frmMyNewCleanup.Show` for new tools. The current launcher opens a fresh form instance with `VBA.UserForms.Add(...)` through `OpenCleanupTool`, which avoids the old "form already displayed" problem and keeps return-to-main behavior consistent.
...
- Apply and Reset are the only bottom action buttons
