...
See the repository's `docs/Release_Notes_v0.9.0-alpha.md` for release highlights,
...
1. Merge the verified release commit into `main` and confirm the Build and Test
   workflow passes.
2. Create and push tag `v0.9.0-alpha` at that exact commit.
3. Create a GitHub Release for the tag and attach `CleanupSuite.dotm` with that
   ...
   resulting `/releases/download/v0.9.0-alpha/CleanupSuite.dotm` URL.
